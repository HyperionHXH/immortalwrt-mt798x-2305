...
- `honor_fur-602` 已从 21.02 的旧 `gsw` 写法改成 23.05 风格的 DSA/`mt7531` 交换机描述，LAN/WAN 由 `lan1 lan2 lan3` 和 `wan` 生成，并补齐 DSA HNAT。2026-07-26 的实机测试已确认设备完成启动、网口以 1 Gbps 建链、IPv4 DHCP 正常分配地址、`192.168.1.1` ping 正常，LuCI HTTP 和 SSH 均可用；`mt_wifi` 已加载，2.4 GHz `ra0` 和 5 GHz `rax0` 均启动并生成默认 SSID。当前 Action 产物曾漏带 RAX3000M 已使用的 scut 兼容修复；只替换 `scutclient.lua` 后，LuCI 根页以及 scut 设置、状态、日志、关于页面全部正常，证明该异常不是 FUR602 DTS/DSA 故障。无线客户端实际连接、WAN、其余 LAN 口映射、按钮、MAC 和 sysupgrade 仍需继续实机验证。
