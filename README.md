...
超过 4 GiB 时还会直接以 `System ERROR: E_FAIL` 失败。所以打包步骤按体积二选一：

- **装得下（≤ 1.4 GiB）**：依旧打成一个整包
  `immortalwrt-<变种>-scutclient-<日期>.7z`，文件名和以前完全一致。
  `mt7986-ax4200`、`mt7986-ax6000-256m`、`mt7986-ax6000` 都走这条路。
- **装不下**：按设备拆成多个小包
  `immortalwrt-<变种>-scutclient-<日期>-<设备名>.7z`，下载自己那台设备的包即可，
  不必为了一个几十兆的镜像拉几个 G。目前只有 `mt7981-ax3000` 走这条路——它会一次
  编出 50+ 个设备的镜像（约 4.3 GiB，单个镜像约 76 MB）。
...
如果 FUR602 启动后常亮绿灯但 LAN 无法访问，先不要继续刷机。将电脑用网线直连路由器，同时保留 Wi-Fi 上网，以管理员身份打开 PowerShell，在仓库目录运行：
